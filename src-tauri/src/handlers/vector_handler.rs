...
#[derive(Deserialize)]
struct CreateTableBody { name: String, dimension: u32, vec_type: String }

async fn vec_create_table(Json(b): Json<CreateTableBody>) -> Json<super::EmptyResponse> {
    api_empty!(crate::vector::vec_create_table(b.name, b.dimension, b.vec_type))
}

#[derive(Deserialize)]
struct InsertBody { table: String, id: String, vector: Vec<f32>, vec_type: String }

async fn vec_insert(Json(b): Json<InsertBody>) -> Json<super::EmptyResponse> {
    api_empty!(crate::vector::vec_insert(b.table, b.id, b.vector, b.vec_type))
}

#[derive(Deserialize)]
struct SearchBody { table: String, query_vector: Vec<f32>, top_k: usize, vec_type: String }

async fn vec_search(Json(b): Json<SearchBody>) -> Json<super::ApiResponse<Vec<crate::vector::SearchResult>>> {
    api_ok!(crate::vector::vec_search(b.table, b.query_vector, b.top_k, b.vec_type))
}

#[derive(Deserialize)]
struct VecDeleteBody { table: String, id: String, vec_type: String }
...
#[derive(Deserialize)]
struct TablesQuery { vec_type: String }
...
#[derive(Deserialize)]
struct TableInfoQuery { table: String, vec_type: String }

async fn vec_table_info(Query(q): Query<TableInfoQuery>) -> Json<super::ApiResponse<crate::vector::TableInfo>> {
